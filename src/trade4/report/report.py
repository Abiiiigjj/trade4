...
def _bootstrap_ci(
    ...
    """95% bootstrap CI on mean cycle return."""
    if len(values) < 2:
        return float("nan"), float("nan")
    arr = np.array(values)
    rng = np.random.default_rng(42)
    boot_means = [
        float(np.mean(rng.choice(arr, len(arr), replace=True))) for _ in range(n_boot)
    ]
...
def generate_report(
    ...
    for symbol, result in backtest_results.items():
        ...
        processed[symbol] = {
            ...
            "avg_pnl_per_cycle": float(np.mean(pnls)) if pnls else 0.0,
            "ci_lo": ci_lo,
            "ci_hi": ci_hi,
...
def _compute_sensitivity(result: BacktestResult) -> SensitivityResult:
    ...
    def net_edge(funding: float, fee_mult: float, slip_mult: float) -> float:
        # Sensitivity uses FDUSD maker-only cost model (our actual execution model)
        m = CostModel(
